...
public partial class GetBook : System.Web.UI.Page
{
    ...
    public void pagebind()
    {
        ...
        String str = "select Member.username,Member.CardNum,Member.tel,Book.ISBN,Book.vname,Book.Pbulish,RentA.Renttime,RentA.ID from RentA,Book,Member where RentA.ISBN=Book.ISBN and RentA.CardNum=Member.CardNum and "+
            "RentA.ISBN='"+this.isbn.SelectedItem.Value.ToString()+"' and RentA.CardNum='"+this.textbox.Text.ToString().Trim()+"' and RentA.Backtime is null";
    ...
    protected void dLGoodsList_ItemCommand(object source, DataListCommandEventArgs e)
    {
        ...
        String sqlstr = "update RentA set Backtime='"+time+"' where ID='"+id+"'";
    ...
    private void Bindname()
    {
        SqlConnection sqlconn = DBHelper.GetSqlConnection();
        sqlconn.Open();
        String sqlstr = "select Book.* from Book,Class where Book.ClassID=Class.ID and Class.Classname=N'" + Clas.SelectedItem.Text.ToString().Trim() + "'";
        DataSet daset = DBHelper.GetDataSet(sqlstr, sqlconn);
    ...
    private void updateBook(int id) {
        int temp = 0;
        String sqlstr = "select Book.* from Book,RentA where RentA.ID='" + id + "' and RentA.ISBN=Book.ISBN";
        SqlConnection sqlconn = DBHelper.GetSqlConnection();
        sqlconn.Open();
    ...
    private void updateMemeber(int id)
    {
        int temp = 0;
        String sqlstr = "select Member.* from Member,RentA where RentA.ID='" + id + "' and RentA.CardNum=Member.CardNum";
        SqlConnection sqlconn = DBHelper.GetSqlConnection();
        sqlconn.Open();
        SqlDataReader sqlreader = DBHelper.GetSqlDataReader(sqlstr, sqlconn);
        while (sqlreader.Read())
        ...
        sqlconn.Close();
        sqlconn = null;
        String sqlstr1 = "update Member set Rnum=Rnum-1 where ID='" + temp + "'";
        DBHelper.DELE_UPDA(sqlstr1);
    }
}